...
/** PBA box score short codes → franchise slug */
export const PBA_TEAM_CODE_TO_SLUG: Record<string, string> = {
  BWB: "blackwater",
  CON: "converge",
  GIN: "ginebra",
  MAG: "magnolia",
...
const MACAU_VARIANTS = {
  MGP: { name: "Macau Giant Pandas", shortName: "MGP" },
  MBK: { name: "Macau Black Bears", shortName: "MBK" },
} as const;

const PBA_TEAM_META: Record<string, { name: string; shortName: string; accentColor: string }> = {
  blackwater: { name: "Blackwater Bossing", shortName: "BWB", accentColor: "#FF6600" },
  converge: { name: "Converge FiberXers", shortName: "CON", accentColor: "#00AEEF" },
  ginebra: { name: "Barangay Ginebra San Miguel", shortName: "GIN", accentColor: "#CE1126" },
  magnolia: {
    name: "Magnolia Chicken Timplados Hotshots",
    shortName: "MAG",
    accentColor: "#7D2248",
  },
  meralco: { name: "Meralco Bolts", shortName: "MER", accentColor: "#F58220" },
  macau: { name: "Macau Giant Pandas", shortName: "MGP", accentColor: "#DC143C" },
  nlex: { name: "NLEX Road Warriors", shortName: "NLX", accentColor: "#FF6B00" },
  phoenix: { name: "Phoenix Super LPG Fuel Masters", shortName: "PHX", accentColor: "#0072CE" },
  rainorshine: { name: "Rain or Shine Elasto Painters", shortName: "ROS", accentColor: "#FFD700" },
  smb: { name: "San Miguel Beermen", shortName: "SMB", accentColor: "#0B3D91" },
  terrafirma: { name: "Terrafirma Dyip", shortName: "TER", accentColor: "#00843D" },
  titan: { name: "Titan Ultra Giant Risers", shortName: "TGR", accentColor: "#512888" },
  tnt: { name: "TNT Tropang 5G", shortName: "TNT", accentColor: "#FFD100" },
};

export type PbaStatsRow = {
  game_id: string;
  game_date: string;
  venue: string;
  team: string;
...
export type PbaImportConfig = {
  ...
  sourcePath: string;
  outPath: string;
  /** Team codes present in this conference export (defaults to all mapped codes). */
  teamCodes?: string[];
  /** Macau guest team label for this conference. */
  macauVariant?: keyof typeof MACAU_VARIANTS;
};

export function buildPbaSeasonTeams(
  seasonId: string,
  teamCodes?: string[],
  macauVariant: keyof typeof MACAU_VARIANTS = "MGP"
): Team[] {
  const codes = teamCodes ?? [...new Set(Object.keys(PBA_TEAM_CODE_TO_SLUG))];
  const slugs = [...new Set(codes.map((code) => PBA_TEAM_CODE_TO_SLUG[code.toUpperCase()]))];

  return slugs.map((slug) => {
    const meta = { ...PBA_TEAM_META[slug] };
    if (slug === "macau") {
      Object.assign(meta, MACAU_VARIANTS[macauVariant]);
    }
    return {
      id: `${slug}-${seasonId}`,
      name: meta.name,
...
function teamIdForCode(code: string, seasonId: string): string {
  const slug = PBA_TEAM_CODE_TO_SLUG[code.toUpperCase()];
  if (!slug) throw new Error(`Unknown PBA team code: ${code}`);
...
function getTeamRef(id: string, allTeams: Team[]): Team {
  ...
  if (!team) throw new Error(`Missing PBA team: ${id}`);
  return { ...team };
}

export function importPbaStats(rows: PbaStatsRow[], config: PbaImportConfig) {
  const allTeams = buildPbaSeasonTeams(
    config.seasonId,
    config.teamCodes,
    config.macauVariant ?? "MGP"
  );
  ...
  for (const row of rows) {
    ...
    const teamId = teamIdForCode(teamCode, config.seasonId);
  ...
  for (const g of games.values()) {
    const awayId = teamIdForCode(g.awayCode, config.seasonId);
    const homeId = teamIdForCode(g.homeCode, config.seasonId);
  ...
  const teamsWithRecords = allTeams.map((team) => ({
    ...team,
  ...
  return {
    gameCount: importedGames.length,
    playerCount: importedPlayers.length,
    teamCount: teamsWithRecords.length,
  };
}
